...
export function runInspect(workspaceRoot, dumpSample = false) {
    ...
    console.log(`workspace state.vscdb: ${workspacePath ?? "(no workspaceStorage entry for this folder — open it in Cursor once)"}`);
    ...
    console.log("(Composer path uses aiService.generations; assistant lines may be FedLearn placeholders unless Cursor persists real replies.)");
    if (dumpSample && turns.length > 0) {
        console.log("\nSample (first 4 turns):");
        for (const t of turns.slice(0, 4)) {
            const snip = t.content.length > 120 ? `${t.content.slice(0, 117)}…` : t.content;
...
export function runWatchCli(args) {
    ...
    console.log(`[fedlearn-watcher] userId=${args.userId} workspace=${args.workspaceRoot} poll=${args.pollMs}ms\n` +
