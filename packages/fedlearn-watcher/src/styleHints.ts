 ...
 */
export function extractStyleHints(recentUserMessages: string[]): string[] {
  ...
  const codeRatio =
    msgs.filter(
      (m) =>
        m.includes("```") ||
        m.includes("    ") ||
        /\b(function|const|let|def|class)\b/.test(m)
    ).length / msgs.length;
  if (codeRatio > 0.3) {
  ...
  const formalCount = msgs.filter((m) =>
    formalSignals.some((s) => m.toLowerCase().includes(s))
  ).length;
  const casualCount = msgs.filter((m) =>
    casualSignals.some((s) => m.toLowerCase().includes(s))
  ).length;
