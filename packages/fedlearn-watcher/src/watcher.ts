...
import { loadWatcherState, saveWatcherState, type WatcherStateFile } from "./stateFile.js";
import { processTurnsIntoPairs, type PendingTurn } from "./pairing.js";
...
function filterTurnsByLookback(turns: CursorTurn[], lookbackMs: number, now: number): CursorTurn[] {
...
export interface FedLearnWatcherOptions {
  ...
  lookbackMs?: number;
  onLearned?: (payload: { pairs: number }) => void;
}

export class FedLearnWatcherRun {
  private reader: CursorReader | null = null;
  private timer: NodeJS.Timeout | null = null;

  constructor(private readonly opts: FedLearnWatcherOptions) {}
  ...
  start(): void {
    ...
    if (!workspacePath) {
      console.warn(
        `[fedlearn-watcher] No workspaceStorage DB matched workspaceRoot=${this.opts.workspaceRoot}; composer extraction may miss aiService.generations.`
      );
    }
  ...
  private async pollOnce(): Promise<void> {
    ...
    const pendingTtl =
      this.opts.pendingTtlWallMs ?? envInt("FEDLEARN_WATCHER_PENDING_TTL_MS", 30_000);

    const now = Date.now();
    let turns: CursorTurn[];
    try {
    ...
    let pendingIn: PendingTurn | null = null;
