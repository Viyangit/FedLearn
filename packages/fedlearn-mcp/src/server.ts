...
type LearnFromTurnToolResponse = {
...
const liveStateByConversation = new Map<string, LiveState>();
const seenTurnIdsByConversation = new Map<string, Set<string>>();
...
async function ensureLiveState(userId: string, conversationId: string): Promise<LiveState> {
...
function inferInputStyle(latestInput: string | undefined): {
  detailPreference: "brief" | "balanced" | "detailed";
  formatPreference: "plain" | "bullet" | "step-by-step";
  toneHint: "direct" | "friendly" | "formal";
} {
  ...
  const detailPreference =
    tokenCount < 8 || hasDirectSignals ? "brief" : tokenCount > 24 ? "detailed" : "balanced";
...
async function getPersonalizationContext(args: PersonalizationContextArgs): Promise<LearnFromTurnToolResponse> {
  ...
  const styleContext =
    `FedLearn context for conversation=${args.conversationId}: ` +
...
async function learnFromTurn(args: TurnArgs): Promise<LearnFromTurnToolResponse> {
  const state = await ensureLiveState(args.userId, args.conversationId);
  const key = conversationKey(args.userId, args.conversationId);
  if (args.turnId) {
    const seen = seenTurnIdsByConversation.get(key) ?? new Set<string>();
        ...
        content: [
          {
            type: "text",
            text:
              `FedLearn ignored duplicate turnId and kept existing state. ` +
    ...
    content: [
      {
        type: "text",
        text:
          `FedLearn learned and applied this turn. ` +
          `sessionsRetained=${state.sessions}, ` +
          `pattern_coverage=${state.pct.toFixed(2)}%, ` +
          `budget=${summary.consumedEpsilon.toFixed(2)}ε/${summary.totalEpsilon.toFixed(2)}ε, ` +
          `rank=${summary.autoAdjustedRankLabel}`
...
async function preTurn(args: PreTurnArgs): Promise<LearnFromTurnToolResponse> {
  const state = await ensureLiveState(args.userId, args.conversationId);
...
async function flushConversation(args: { userId: string; conversationId: string }): Promise<LearnFromTurnToolResponse> {
  const state = await ensureLiveState(args.userId, args.conversationId);
  const summary = state.adapter.memorySummary();
    ...
    content: [
      {
        type: "text",
        text:
          `FedLearn is already applying every turn. ` +
          `sessionsRetained=${state.sessions}, ` +
          `pattern_coverage=${state.pct.toFixed(2)}%, ` +
          `budget=${summary.consumedEpsilon.toFixed(2)}ε/${summary.totalEpsilon.toFixed(2)}ε, ` +
          `rank=${summary.autoAdjustedRankLabel}`
...
const server = new McpServer(
  { name: "fedlearn", version: "1.0.0" },
  {
    instructions:
      "Strict flow required for each response: " +
...
server.registerTool(
  "pre_turn",
  {
    ...
    description:
      "Increments within-session progress immediately so personalization percentage moves before post-turn apply.",
...
server.registerTool(
  "get_personalization_context",
  {
    ...
    description:
      "Returns moderate-token local style context (tone/format/detail/confidence) derived from prior interactions.",
    inputSchema: z.object({
      userId: z.string().optional().describe("Stable user id for local persistence"),
      conversationId: z.string().describe("Stable conversation/thread id"),
      latestInput: z
...
server.registerTool(
  "learn_from_turn",
  {
    ...
    description:
      "Writes a post-answer learning update from the provided user prompt and assistant response.",
    inputSchema: z.object({
      userId: z.string().optional().describe("Stable user id for local persistence"),
      conversationId: z.string().describe("Stable conversation/thread id"),
